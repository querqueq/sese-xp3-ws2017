...
import at.ac.tuwien.student.sese2017.xp.hotelmanagement.util.AnsiCodes;
 ...
 *   {@link Order} annotation can be used
...
public class TestDataInjectorService {
  ...
  public TestDataInjectorService(
      ...
      // Test data entities
      @Autowired(required = false) // Optional, not initialized if testdata=false
  ...
  public void inject() {
    ...
    if(this.td == null) {
      log.error("Test data directory bean not set, aborting");
      return;
    }
    ...
    try {
      ...
      log.info("Unit test detected, injecting test data");
    ...
    String ansiStart = AnsiCodes.GREEN_BACKGROUND +  AnsiCodes.BOLD + AnsiCodes.BLACK;
    String ansiEnd = AnsiCodes.RESET;
    log.info("Loaded {}Testdata v{}{}", ansiStart, td.getTestdataVersion(), ansiEnd);
  ...
  public List<Object> prepareTestData() {
    ...
    Class clazz = td.getClass();

    // Iterate through class and all superclasses
    while (clazz != null) {
      addAllFields(clazz, data);
  ...
  private void addAllFields(Class clazz, Map<Integer, List<Object>> data) {
    // Get all declared fields and
   ...
   */
  private static void addToHashList(int key, Object value,
                                    Map<Integer, List<Object>> priorityList) {
